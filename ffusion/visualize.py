...
def matplotlib_plot(datasets, frames, best_lines, filename=None, labels=None, x='Iteration', y='Objective value', 
    ...
    leg = plt.legend(handles=handles, title=None, loc="lower center", bbox_to_anchor=(-0.1, -0.5), ncol=4, fancybox=True, frameon=True)
    ...
    #plt.axis((None,_maxX,y1*0.995,max(min(y2*1.1,1.0), 2*lhigh)))
...
def matplotlib_plot2(frames, datasets, klist, filename=None, labels=None, x='Factorization rank', y='Number of iterations', 
    ...
    leg = plt.legend(handles=[handles[0], handles[1]], title=None, 
...
def visu_time():
    data = load_file('results/visdata/time.pkl')
    lines = data[0]
    times = data[1]
    titles = data[2]
    klist = data[3]
    ...
    print(lines)
    matplotlib_plot2(lines, titles, klist, y="Factorization time [s]", filename=os.path.join(IMG_DIR, 'factorization_time.png'))
    matplotlib_plot2(times, titles, klist, y="Iteration time [s]", filename=os.path.join(IMG_DIR, 'time_per_iteration.png'))
        #analyze(frames, dataset=dataset)


if __name__ == '__main__':
    visu_convergence(k=10)
#    visu_convergence(k=30)
#    visu_rank()
#    visu_time()